...
class MaterielRepository extends ServiceEntityRepository
{
    ...
    public function findByEmploye($employe_id)
    {
        return $this->createQueryBuilder('r')
            ->join('r.materiel','m')
            ->join('p.employe','e')
            ->Where('e.id = :val')
            ->setParameter('val', $employe_id)
            ->getQuery()
            ->getResult();
    }

    public function findMaterielBySearchTerm($searchTerm): array
    {
       return $this->createQueryBuilder('u')
           ->where('u.nom LIKE :searchTerm OR
            u.marque LIKE :searchTerm OR
            u.num_serie Like :searchTerm OR
    ...
    public function countMaterielbyetat($etat): int
    {
        return $this->createQueryBuilder('r')
            ->select('count(r.id)')
            ->Where('r.etat >= :val')
